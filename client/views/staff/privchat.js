...
import './privchat.html';

Template.privchat.onCreated(function helloOnCreated() {
	
	Meteor.subscribe('chats',50);

	this.selectedChatId = new ReactiveVar(false);

});

	Template.privchat.onRendered(function () {
	...
	selectedClient()
	{	
		return Meteor.users.findOne({_id:Template.instance().selectedChatId.get()});
	},
	activeChats(){
		
			return Chats.find({ctype:1},{limit: 10 ,sort: {timestamp: -1}});
	}, 
	messages() {
...
Template.privchat.events({
	  ...
	  var mes = event.target.message.value;
		...
		Meteor.call("insertChatMessage",mes,Template.instance().selectedChatId.get(),0,1,function(err,res){
        if (!err) {
			Tracker.afterFlush(function () {
